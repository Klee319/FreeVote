...
export class PollsController {
  ...
  static async createPoll(req: Request, res: Response, next: NextFunction) {
    try {
      // デバッグログ追加
      console.log('[DEBUG] createPoll - Request Headers:', req.headers);
      console.log('[DEBUG] createPoll - Request User:', (req as any).user);
      console.log('[DEBUG] createPoll - Request Body:', req.body);
      console.log('[DEBUG] createPoll - DISABLE_AUTH:', process.env.DISABLE_AUTH);
      
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        console.log('[DEBUG] createPoll - Validation Errors:', errors.array());
      ...
      const userId = (req as any).user?.id || req.body.createdBy || 'default-admin';
      console.log('[DEBUG] createPoll - userId:', userId);
      
      if (!userId) {
        console.log('[DEBUG] createPoll - No userId found, returning 401');
      ...
      console.log('[DEBUG] createPoll - Creating poll with data:', {
        ...req.body,
        createdBy: userId,
      ...
      console.log('[DEBUG] createPoll - Poll created successfully:', poll);
   ...
   * 新着投票を取得
   */
  static getRecentPollsValidation = [
    query('limit').optional().isInt({ min: 1, max: 20 }),
  ];

  static async getRecentPolls(req: Request, res: Response, next: NextFunction) {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        return res.status(400).json({
      ...
      const recentPolls = await pollService.getRecentPolls(limit);
      ...
      console.error('[PollsController.getRecentPolls] Error:', error);
